...
function isEmployeeRestDay(employee, date) {
  return normalizeRestDay(employee?.standard_rest_day) === date.getDay()
}

function isDateKeyOnEmployeeRestDay(employee, dateKey) {
  const restDay = normalizeRestDay(employee?.standard_rest_day)
...
function runSelfTests() {
  const tests = [
    ...
    ['isDateKeyOnEmployeeRestDay', isDateKeyOnEmployeeRestDay({ standard_rest_day: 1 }, '2026-05-11') === true],
  ]
...
export default function App() {
  ...
  function collectDateKeysForRange(startKey, endKey, standardRestDay) {
    const single = startKey === endKey
    const restDay = normalizeRestDay(standardRestDay)
    return monthDays
      .filter((date) => {
        const key = fmtDateKey(date)
        if (key < startKey || key > endKey) return false
        if (!single && restDay !== null && date.getDay() === restDay) return false
        return true
      })
  ...
  async function saveEditor() {
    ...
    const targetDateKeys = collectDateKeysForRange(editorState.dateKey, editorState.applyUntil || editorState.dateKey, editorState.standard_rest_day)
