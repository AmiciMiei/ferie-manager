  ...
  const tables = DEPARTMENTS.map((department) => {
    ...
    const body = employees.map((employee) => {
      ...
      const cells = monthDays.map((date) => {
        ...
        const codeClass = entry.code === 'F' ? ' code-f' : ''
        const mergedClass = mondayClass ? ` class="${mondayClass.replace(/ class=\"|\"/g, '')}${codeClass}"` : (codeClass ? ` class="${codeClass.trim()}"` : '')
    ...
    <html lang="it">
      <head>
        ...
        <style>
          ...
          td.code-f { background: #22c55e; color: #fff; font-weight: 800; border-color: #15803d; }
